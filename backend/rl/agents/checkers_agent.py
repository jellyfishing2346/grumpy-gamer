...
class CheckersRLAgent:
        ...
        for dr, dc in [(-1, -1), (-1, 1), (1, -1), (1, 1)]:
            ...
            if 0 <= end_row < 8 and 0 <= end_col < 8:
                ...
                if mid_piece in opponent_pieces and end_piece == self.EMPTY:
                    is_black_king = player == 'black' and end_row == 7
                    is_red_king = player == 'red' and end_row == 0
                    will_king = is_black_king or is_red_king
        ...
        for dr, dc in directions:
            ...
            if 0 <= new_row < 8 and 0 <= new_col < 8:
                if board[new_row, new_col] == self.EMPTY:
                    is_black_king = player == 'black' and new_row == 7
                    is_red_king = player == 'red' and new_row == 0
                    will_king = is_black_king or is_red_king
                    moves.append({
                        'from': (row, col),
                        'to': (new_row, new_col),
    ...
    def _minimax(self, board: np.ndarray, depth: int, alpha: float,
                 beta: float, maximizing: bool) -> Tuple[float, Optional[Dict]]:
