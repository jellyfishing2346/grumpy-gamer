  ...
  return (
    <div style={{ display: "flex", gap: "0.4em", justifyContent: "center", marginTop: "0.8em" }}>
      {guess.split("").map((letter, i) => (
        <div key={i} style={{
          width: 44, height: 44, background: colorMap[feedback[i]] || colorMap.gray,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.5em", marginTop: "0.8em" }}>
      {Object.entries(moveData).map(([key, value]) => (
        <div key={key} style={{ display: "flex", justifyContent: "space-between", padding: "0.5em 0.8em", background: darkMode ? "rgba(255,255,255,0.04)" : "rgba(126,203,255,0.05)", borderRadius: 8 }}>
          <span style={{ color: textMuted, fontSize: "0.88em", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{key}</span>
          <span style={{ color: textPrimary, fontSize: "0.95em", fontWeight: 500 }}>{Array.isArray(value) ? value.join(", ") : String(value)}</span>
        </div>
      ))}
    </div>
  );
...
const SpectatorViewer: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchReplay() {
      setLoading(true);
      try {
        const res = await fetch(`${API_URL}/api/replays/public/${session_id}`);
        if (res.ok) {
  ...
  useEffect(() => {
    if (isPlaying && session) {
      intervalRef.current = setInterval(() => {
        setCurrentMove(prev => {
          if (prev >= session.moves.length - 1) { setIsPlaying(false); return prev; }
          return prev + 1;
        });
  ...
  const handlePlayPause = () => {
    if (session && currentMove >= session.moves.length - 1) {
      setCurrentMove(0); setIsPlaying(true);
    } else {
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      {/* Header */}
      <div style={{
        ...
        <div style={{ marginTop: "1.2em" }}>
          <button onClick={() => navigate("/signup")} style={{ padding: "0.5em 1.4em", borderRadius: 50, border: "none", background: "linear-gradient(90deg, #7ecbff, #4fa3d1)", color: "#1a1a2e", fontWeight: 700, fontSize: "0.9em", cursor: "pointer", fontFamily: "inherit" }}>
            Create your own replays →
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "48px 24px 64px" }}>
        ...
        ) : notFound || !session || totalMoves === 0 ? (
          <div style={cardStyle({ textAlign: "center", padding: "3em" })}>
            <div style={{ fontSize: "2.5em", marginBottom: "0.5em" }}>🔍</div>
            <h2 style={{ color: "#7ecbff", marginBottom: "0.5em" }}>Replay not found</h2>
            <p style={{ color: textMuted, marginBottom: "1.5em" }}>This replay doesn't exist or has no recorded moves.</p>
            <button onClick={() => navigate("/")} style={{ padding: "0.75em 2em", borderRadius: 50, border: "none", background: "linear-gradient(90deg, #7ecbff, #4fa3d1)", color: "#1a1a2e", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              Go to Grumpy Gamer
            </button>
          </div>
        ) : (
          <>
            ...
            <div style={cardStyle({ padding: "1.2em 1.8em" })}>
              <div style={{ display: "flex", alignItems: "center", gap: "1em", flexWrap: "wrap" }}>
                <button onClick={handlePlayPause} style={{ padding: "0.6em 1.4em", borderRadius: 10, border: isPlaying ? "1px solid rgba(255,126,103,0.3)" : "none", background: isPlaying ? "rgba(255,126,103,0.15)" : "linear-gradient(90deg, #7ecbff, #4fa3d1)", color: isPlaying ? "#ff7e67" : "#1a1a2e", fontWeight: 700, fontSize: "1em", cursor: "pointer", fontFamily: "inherit" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5em" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.3em" }}>
                    {SPEEDS.map((s, i) => (
                      <button key={s.label} onClick={() => setSpeedIndex(i)} style={{ padding: "0.3em 0.7em", borderRadius: 8, fontSize: "0.82em", fontWeight: 600, cursor: "pointer", fontFamily: "inherit", background: speedIndex === i ? "rgba(126,203,255,0.15)" : "transparent", color: speedIndex === i ? "#7ecbff" : textMuted, border: speedIndex === i ? "1px solid rgba(126,203,255,0.3)" : "1px solid rgba(126,203,255,0.1)" }}>{s.label}</button>
                    ))}
                  </div>
                </div>
              </div>
...
export default SpectatorViewer;